...
including Datalog, JSONiq, Einstein summation (Einsum), GraphQL, and more
...
% \begin{lstlisting}
% what are some simpler examples?
% sum(data.*.value)
...
\caption{}
\end{subfigure}
\caption{\todo{caption}}\label{fig:intro}
\end{figure}
...
\todo{mention the fact that what is meant by \inline{sum(data.*.val)} varies by the context}
...
\todo{point to IR section and say that the use of *A will make sure that
this inner query is hoisted out, etc.}
...
In \Cref{sec:intro} (\Cref{fig:intro}), we gained a preliminary understanding
