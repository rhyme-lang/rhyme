...
\subcaption{\todo{caption}}~\label{fig:intro_query}
\end{subfigure}
...
JavaScript code.\todo{caption}}~\label{fig:ir_code}
\end{subfigure}
...
but, the general idea here is that \inline{*} and \inline{*A} are iterators, and 
\inline{sum(data.*.val)} nested inside \inline{data.*.key} signifies the grouping of objects by the
\inline{key} and the subsequent computation of partial aggregates within these groups.
Conversely, since \inline{sum(data.*A.val)} is not nested within another \inline{*A}, it indicates
...
the query.
\inline{tmp0}, \inline{tmp1} and \inline{tmp2} corresponds to intermediate values
...
For instance, the expression \inline{sum(data.*A.val)} represents a nested sub-query,
...
\todo{perhaps show an example of end-to-end workflow first so that the readers know what is coming in
the next few sections}

\todo{Two figures in the intro. 1. showing a data query, visualization query, a tensor computation (transpose?)
2. show end to end workflow for the data query}
