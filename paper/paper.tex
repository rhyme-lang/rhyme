...
\inline{data.*.val} is called a \emph{path} expression and this notation is inspired by JSONPath~\cite{jsonpath}.
...
\begin{itemize}
    ...
    \item We highlight the versatility of \lang{} across various use cases, including the expression
          ...
          tensor computations (akin to Einsum), and the enabling `pipe' APIs via metaprogramming (\Cref{sec:case_studies}).
...
The formal grammar is shown in \Cref{fig:grammar} and
...
% \text{ScalarOp} &::= \textsf{get} \mid \textsf{apply} \mid \textsf{plus} \mid \textsf{minus} \mid \textsf{div} \mid \textsf{fdiv} \mid \textsf{times} \mid \textsf{mod} \\ 
% \text{ReductionOp} &::= \textsf{sum} \mid \textsf{count} \mid \textsf{max} \mid \textsf{min} \mid \textsf{first} \mid \textsf{last} \mid \textsf{array} \\[4pt]
% \text{Atom} &::= \text{Ident} \mid \text{Num} \mid \text{Var} \\
% \text{Path} &::= \text{Atom ( . Atom )*} \\
% \text{Expr} &::= \text{Path} \mid \text{Expr} \ \text{ScalarOp} \ \text{Expr} \mid \text{ReductionOp ( Expr )} \\
...
% \vspace{-7mm}
\caption{Syntax for expressing \lang{} queries. \Cref{sec:query_language} shows how to use this to express
...
named iterators (e.g., \inline{*A}, \inline{*B}, etc.).
...
As previously discussed, since \inline{sum('data.*A.country')} is not nested within a
...
The main query conducts a group-by based on the \inline{region} (retrieved using \inline{get}) initially,
followed by another group-by based on \inline{country}, ultimately computing the
desired aggregates.
...
\vspace{-2mm}
As we saw in \Cref{fig:intro_overview,fig:intro}, all the queries we
...
\lang{} query frontend (JSON) we saw in \Cref{sec:query_language} describes the query
...
\subsection{Combining UI logic}\label{subsec:view}
...
% \subsection{Expressing Visual Components}
