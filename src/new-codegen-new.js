...
function buildDeps(assignmentStms, generatorStms, tmpVarWriteRank) {
  ...
  let stmt2loops = Object.keys(stmtdeps).map(s => Object.keys(loopdeps[s]))
...
exports.generate = (ir, backend = "js") => {
  ...
  let getLoops1 = s => Object.keys(loopdeps[s])
  ...
  function stmtAvailable(curr, dep) {
    return emittedStmts[dep] && stmtClosed(curr, dep)
  }
  ...
  function inLoops(e) {
    return getLoops1(e).every(isOpened)
  }
  ...
  function emitLoopEpilog() {
    ...
    for (let i in closedLoopByStmt) {
      ...
      // console.log(Object.keys(loopdeps[i]))
      if (getLoops1(i).includes(l) && closedLoopByStmt[i][l] === false) {
