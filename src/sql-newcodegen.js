...
let emitStmInit = (q, sym) => {
  ...
  if (q.key == "stateful") {
    ...
    let keySchema = mksetVarEnv[q.arg[1].op].map(key => key.schema.type)
    hashMapInit(buf, sym, keySchema, q.schema.type.objValue)
  } else {
    throw new Error("unknown op: " + pretty(q))
  }
...
let emitCode = (q, ir) => {
  ...
  for (let i in assignments) {
    ...
    if (q.key == "update") {
      ...
      if (q.arg[3].arg[0].arg[0].key == "pure" && q.arg[3].arg[0].arg[0].op == "combine") {
        keySchema = q.arg[3].arg[0].arg[0].arg.map(e => e.schema.type)
      } else {
        keySchema = [q.arg[3].arg[0].arg[0].schema.type]
      }
      hashMapEnv[sym] = { keySchema, valSchema: q.schema.type.objValue }
  ...
  for (let i in assignments) {
    let sym = groupedOps[i] ? groupedOps[i] : tmpSym(i)
    ...
    if (q.key == "stateful" && q.fre.length != 0) {
      ...
      let buf = []
      hashMapInit(buf, sym, keySchema, q.schema.type)
      assign(buf, sym, [], [])
    } else if (q.key == "update" && trivialUpdate[sym] !== undefined) {
      let keySchema = mksetVarEnv[q.arg[1].op].map(key => key.schema.type)
      let buf = []
      hashMapShallowCopy(buf, sym, trivialUpdate[sym], keySchema, q.schema.type.objValue)
...
//
// A different approach of codegen
//
...
let emitPath = (buf, q) => {
  if (q.key == "loadInput") {
    ...
    if (e1.key == "ref") {
      ...
      let key = codegen(e2, buf)
      let keyPos = hashLookUp(buf, sym, [key])[1]
      let { valsSchema } = hashMapEnv[sym]
      if (valsSchema.length > 1) {
        throw new Error("not supported for now")
      }
      if (typing.isString(valsSchema[0].schema)) {
        return { str: `${sym}_${valsSchema[0].name}_str[${keyPos}]`, len: `${sym}_${valsSchema[0].name}_len[${keyPos}]` }
      } else {
        return `${sym}_${valsSchema[0].name}[${keyPos}]`
      }
    }
...
let emitHashMapInit = (buf, sym, keySchema, valSchema) => {
  ...
  for (let i in valSchema) {
    let name = valSchema[i].name
    let schema = valSchema[i].schema
...
let emitCodeNew = (q, ir) => {
  ...
  for (let i in assignments) {
    ...
    if (v.key == "pure" && v.op == "mkTuple") {
      for (let i = 0; i < q.arg[2].arg.length; i += 2) {
        ...
        valsSchema.push({ name: key.op, schema: val.schema.type })
      }
    } else {
      ...
      valsSchema.push({ name: "values", schema: v.schema.type })
    }

    // Create hashmap
    let buf = []
    emitHashMapInit(buf, sym, keySchema, valsSchema)
    assign(buf, sym, [], [])

    hashMapEnv[sym] = { keySchema, valsSchema }
  }
  ...
  for (let i in assignments) {
    ...
    if (initRequired[q.op]) {
      // init
      assign(["// init " + sym + " = " + pretty(q)], sym, q.fre, [])
    }

    let deps = [...union(q.fre, q.bnd), ...q.tmps.map(tmp => assignmentToSym[tmp] ? assignmentToSym[tmp] : tmpSym(tmp))] // XXX rhs dims only?

    // update
    assign(["// update " + sym + " = " + pretty(q)], sym, q.fre, deps)
  }
